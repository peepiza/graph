...
// Функция для корректного декодирования URL (включая %0A)
string url_decode(const string& str) {
    string result;
    for (size_t i = 0; i < str.length(); i++) {
        if (str[i] == '%' && i + 2 < str.length()) {
            // Преобразуем %XX в символ
            char hex[3] = {str[i+1], str[i+2], 0};
            char decoded_char = (char)strtol(hex, nullptr, 16);
            result += decoded_char;
            i += 2;
        } else if (str[i] == '+') {
            result += ' ';
...
string get_html() {
    // ... (ваш HTML код) ...
    // Советую пока оставить ваш старый HTML, но ошибка была именно в парсинге.
    // Для краткости оставлю ваш же HTML, который был в final_server.cpp.
    // Главное — мы исправляем парсинг.
    return "<!DOCTYPE html>\n"
"<html>\n"
...
int main() {
    ...
    while (true) {
        ...
        if (req.find("GET / ") != string::npos || req.find("GET / HTTP") != string::npos) {
            ...
            graph_data_encoded = extract("graph");
            string w = extract("width");
            string h = extract("height");
            string r = extract("radius");
            string d = extract("directed");

            // --- КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: Декодируем строку графа ---
            string graph_data = url_decode(graph_data_encoded);
            ...
            // Отладочный вывод (чтобы убедиться, что данные корректны)
            cout << "DEBUG: Received graph data:\n" << graph_data << endl;
            cout << "--- END DEBUG ---" << endl;

            // Сохраняем в файл
            string input_file = "/tmp/graph_in.txt";
            ofstream f(input_file);
            f << graph_data; // Теперь здесь будут нормальные переносы строк!
            f.close();
